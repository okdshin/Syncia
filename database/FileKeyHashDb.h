...
inline auto CreateFileKeyHashDbWithBasic(boost::asio::io_service& io_service, 
	...
	const auto adder = 
		FileKeyHashAdder([file_key_hash_list, arg_adder](const FileKeyHash& file_key_hash){
			/*
			const auto iter = 
	...
	const auto remover = 
		FileKeyHashRemover(
				[file_key_hash_list, arg_remover](boost::function<bool (const FileKeyHash&)> decider){
			const auto end = 
				std::remove_if(
...
inline auto CreateStandardFileKeyHashDb(boost::asio::io_service& io_service) -> FileKeyHashDb::Ptr {
	auto file_key_hash_list = 
		boost::shared_ptr<FileKeyHashList>(new FileKeyHashList());
...
inline auto CreateBasicFileKeyHashDb(boost::asio::io_service& io_service) -> FileKeyHashDb::Ptr {
	return CreateFileKeyHashDbWithBasic(
		io_service, FileKeyHashAdder(), 
		FileKeyHashRemover());
